#!/bin/bash
# Open VSX 发布脚本 (Linux/macOS版本)
# 用于将三层记忆系统MCP服务器发布到 Open VSX Registry

# 配置信息
EXTENSION_NAME="memory-mcp-server-anderson"
VERSION="1.6.0"
...
check_existing_extension() {
    ...
    if ovsx get "${PUBLISHER}.${EXTENSION_NAME}" &> /dev/null; then
        ...
        # 获取最新版本
        latest_version=$(ovsx get "${PUBLISHER}.${EXTENSION_NAME}" | node -e "console.log(JSON.parse(require('fs').readFileSync(0, 'utf-8')).version)")
