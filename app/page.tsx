...
import { Scissors, Dog, Stethoscope, Sparkles, Wrench, Grid, X, MapPin, Star, Phone, LayoutGrid, List } from "lucide-react";
...
const MOCK_COMPANIES = [
  {
    ...
    description: "Cortes modernos, barba na toalha quente e o ambiente mais descontraído do centro. Agende agora o seu horário com nossos mestres.",
    services: [
    ...
    staff: [
      { id: "s1", name: "João Silva", image: "https://images.unsplash.com/photo-1599566150163-29194dcaad36?w=100&auto=format&fit=crop&q=60" },
      { id: "s2", name: "Marcio", image: "https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100&auto=format&fit=crop&q=60" },
    ]
  },
  {
    id: 2,
    ...
    description: "O maior cuidado e carinho para o seu melhor amigo. Banho, tosa, hidratação e consultório veterinário.",
    services: [
    ...
    staff: [
      { id: "s3", name: "Dra. Ana", image: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=100&auto=format&fit=crop&q=60" },
      { id: "s4", name: "Carlos", image: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=100&auto=format&fit=crop&q=60" },
    ]
  },
  {
    id: 3,
    ...
    description: "Tratamentos faciais e corporais com tecnologia de ponta e profissionais altamente qualificados.",
    services: [
  ...
  },
  {
    ...
    icon: Stethoscope,
    image: "https://images.unsplash.com/photo-1538108149393-ce90eddc5db7?auto=format&fit=crop&q=80&w=800",
    logo: "https://images.unsplash.com/photo-1579684385127-1ef15d508118?auto=format&fit=crop&q=80&w=150",
    description: "Atendimento médico humanizado com especialistas em diversas áreas. Sua saúde em primeiro lugar.",
    services: [
  ...
  },
  {
    ...
    logo: "https://images.unsplash.com/photo-1504222490345-c075b6008014?auto=format&fit=crop&q=80&w=150",
    description: "Injeção eletrônica, suspensão, freios e motor. Serviço transparente e garantia de peças genuínas.",
    services: [
      { name: "Troca de Óleo", price: "R$ 150,00", duration: "40 min" },
      { name: "Alinhamento", price: "R$ 80,00", duration: "1h" },
    ]
  },
  {
    id: 6,
    ...
    description: "Tudo que o seu gatinho ou cachorro precisa. Temos serviço de táxi dog gratuito na região.",
    services: [
  ...
  },
  {
    ...
    description: "A clássica barbearia dos anos 50 no coração da Vila Madalena. Cerveja gelada e sinuca enquanto você espera.",
    services: [
  ...
  },
  {
    ...
    description: "Um oásis de tranquilidade no meio da cidade. Massagens shiatsu, pedras quentes e aromaterapia.",
    services: [
      { name: "Massagem Pedras", price: "R$ 180,00", duration: "1h 20m" },
      { name: "Day Spa Completo", price: "R$ 350,00", duration: "3h" },
    ],
    staff: [
      { id: "s5", name: "Mariana", image: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=100&auto=format&fit=crop&q=60" }
    ]
  }
];
...
const enhancedMockCompanies = MOCK_COMPANIES.map(company => ({
  ...company,
  staff: company.staff ? company.staff.map((s, i) => ({
    ...s,
    availability: i % 2 === 0 ? ["08:00", "09:30", "13:00", "14:30", "16:00"] : ["10:00", "11:00", "15:00", "17:30", "18:15"]
  })) : [
    { id: "f1", name: "Profissional 1", image: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&auto=format&fit=crop&q=60", availability: ["08:00", "10:00", "14:00"] },
    { id: "f2", name: "Profissional 2", image: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=100&auto=format&fit=crop&q=60", availability: ["09:00", "11:00", "15:00", "16:30"] }
  ]
}));

export default function Home() {
  const [selectedCompany, setSelectedCompany] = useState<typeof enhancedMockCompanies[0] | null>(null);
  ...
  const filteredCompanies = enhancedMockCompanies.filter(company => {
    const matchesCategory = activeCategory === "Todos" || company.niche === activeCategory;
  ...
  const handleOpenCompany = (company: typeof enhancedMockCompanies[0]) => {
    ...
    setSelectedProfessional(company.staff && company.staff.length > 0 ? company.staff[0].id : null);
    setSelectedTime(null);
  };

  const closeModal = () => setSelectedCompany(null);

  const activeProfessionalData = selectedCompany?.staff?.find(p => p.id === selectedProfessional);
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a]">
      ...
      <main className="flex flex-col">
        ...
        <section className="relative flex flex-col items-center justify-center overflow-hidden bg-black py-20 text-center dark:bg-black w-full transition-all duration-300">
          ...
          <div className="container px-4 z-10">
            ...
            <h1 className="mx-auto max-w-3xl text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">
              O que você quer <span className="text-primary">agendar</span> hoje?
            </h1>

            <div ref={searchContainerRef} className={`mt-10 mx-auto w-full max-w-7xl transition-all duration-500 ease-in-out ${isSticky ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
              ...
              <div className="flex flex-row overflow-x-auto justify-start xl:justify-center items-center gap-4 w-full py-2 scrollbar-hide px-4 md:px-0 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:none]">
                {CATEGORIES.map((cat) => (
                  ...
                  >
                    <cat.icon className={`w-5 h-5 ${activeCategory === cat.name ? 'opacity-100' : 'opacity-70 text-primary'}`} />
        ...
        <div
          className={`fixed top-16 left-0 right-0 z-30 w-full bg-[#050505] border-b border-white/10 shadow-2xl py-3 transform transition-all duration-300 ease-in-out ${isSticky ? 'translate-y-0 opacity-100' : '-translate-y-full opacity-0 pointer-events-none'
            }`}
        >
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 justify-between">
              ...
              <div className="flex items-center gap-2 overflow-x-auto w-full lg:w-2/3 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:none]">
                {CATEGORIES.map((cat) => (
                  ...
                  >
                    <cat.icon className={`w-3.5 h-3.5 ${activeCategory === cat.name ? 'opacity-100' : 'opacity-70 text-primary'}`} />
        ...
        <section className="container mx-auto px-4 py-16 sm:px-6 lg:px-8 min-h-screen">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">
              {activeCategory === "Todos" ? "Empresas em Destaque" : `${activeCategory} em Destaque`}
            </h2>
          ...
          {filteredCompanies.length > 0 ? (
            <div className={layoutMode === "grid"
              ...
              {filteredCompanies.map((company) => (
                ...
                >
                  {/* Image Header */}
                  <div className={`${layoutMode === "grid" ? "h-32 w-full" : "h-full w-[250px] shrink-0"} relative bg-gray-200 dark:bg-gray-800`}>
                    <img src={company.image} alt="Banner" className="w-full h-full object-cover" />
                  ...
                  {/* Card Content */}
                  <div className={`flex flex-col flex-1 p-6 ${layoutMode === "grid" ? "pt-10" : ""}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-lg font-bold text-gray-900 dark:text-white group-hover:text-primary transition-colors line-clamp-1">{company.name}</h3>
                    </div>
                    <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-4">{company.niche}</p>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2 mt-3">
                          {company.services.slice(0, 3).map((s, idx) => (
                            <span key={idx} className="text-xs font-semibold bg-gray-100 dark:bg-gray-800/60 text-gray-600 dark:text-gray-300 px-2.5 py-1 rounded-md border border-gray-200 dark:border-gray-700/50">
                              {s.name}
                            </span>
                          ))}
                        </div>
            ...
            <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50/50 p-16 text-center dark:border-gray-800 dark:bg-gray-900/50">
              <h3 className="text-xl font-medium text-gray-900 dark:text-white">Nenhum resultado encontrado</h3>
      ...
      {selectedCompany && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-sm transition-opacity" onClick={closeModal}>
          <div
          ...
          >
            ...
            <div className="relative flex-1 overflow-y-auto pb-6 w-full custom-scrollbar">
              ...
              <div className="absolute top-0 left-0 h-1 bg-primary z-50 transition-all duration-500" style={{ width: selectedTime ? '100%' : selectedProfessional ? '66%' : selectedService ? '33%' : '0%' }}></div>

              {/* Modal Header/Banner */}
              <div className="h-40 sm:h-48 w-full relative bg-gray-800">
                <img src={selectedCompany.image} alt="Banner" className="w-full h-full object-cover opacity-80" />
              ...
              <div className="px-6 relative -mt-16">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="mt-4">
                      ...
                      <div className="flex items-center gap-3">
                        <p className="text-primary font-medium text-sm">{selectedCompany.niche}</p>
                ...
                {/* 1. Services */}
                <h3 className="text-lg font-bold text-white mb-3">1. Escolha um Serviço</h3>
                <div className="space-y-3 mb-8">
                  {selectedCompany.services.map((service, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-colors ${isSelected ? 'border-primary' : 'border-gray-600'}`}>
                            {isSelected && <div className="w-2.5 h-2.5 bg-primary rounded-full"></div>}
                          </div>
                          <div>
                            <h4 className={`font-semibold text-sm transition-colors ${isSelected ? 'text-primary' : 'text-gray-200 group-hover:text-white'}`}>{service.name}</h4>
                            <p className="text-xs text-gray-500 mt-0.5">{service.duration}</p>
                          </div>
                        </div>
                        <p className={`font-bold text-sm ${isSelected ? 'text-primary' : 'text-gray-300'}`}>{service.price}</p>
                      </div>
                    )
                  })}
                </div>
                ...
                <div className={`transition-all duration-500 ease-in-out ${selectedService ? 'opacity-100 max-h-[500px]' : 'opacity-50 max-h-40 overflow-hidden pointer-events-none'}`}>
                  <h3 className="text-lg font-bold text-white mb-3">2. Profissional</h3>
                  <div className="flex items-center gap-4 overflow-x-auto pb-4 mb-6 scrollbar-hide py-3 px-2">
                    {selectedCompany.staff?.map((person) => {
                      ...
                      return (
                        <div
                          key={person.id}
                          onClick={() => { setSelectedProfessional(person.id); setSelectedTime(null); }}
                        ...
                        >
                          <div className={`h-16 w-16 rounded-full overflow-hidden transition-all ${isSelected ? 'ring-2 ring-offset-4 ring-primary ring-offset-[#111] shadow-[0_0_15px_rgba(0,255,255,0.3)]' : 'border-2 border-transparent group-hover:border-gray-500'}`}>
                            <img src={person.image} alt={person.name} className="w-full h-full object-cover" />
                          </div>
                          <span className={`text-xs font-medium transition-colors ${isSelected ? 'text-primary font-bold' : 'text-gray-400 group-hover:text-gray-200'}`}>
                            {person.name}
                          </span>
                        </div>
                      )
                ...
                {/* 3. Time Slots - Only show if Professional is selected */}
                <div className={`transition-all duration-500 ease-in-out ${selectedProfessional ? 'opacity-100 max-h-[500px]' : 'opacity-0 max-h-0 overflow-hidden pointer-events-none'}`}>
                  <h3 className="text-lg font-bold text-white mb-3">3. Horários Livres Hoje</h3>
                  ...
                  {activeProfessionalData && (
                    <div className="flex flex-wrap gap-3 mb-8">
                      {activeProfessionalData.availability?.length > 0 ? (
                        ...
                        <p className="text-sm text-gray-500 italic">Nenhum horário disponível para hoje.</p>
                      )}
                    </div>
                  )}
                </div>
                ...
                {/* Checkout Button */}
                <div className={`sticky bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0a0a0b] via-[#0a0a0b] to-transparent transition-all duration-500 ${selectedTime ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
                  <Link href="/register/client" className="w-full block">
                    <button className="w-full bg-primary text-black font-bold py-4 rounded-xl shadow-[0_0_20px_rgba(0,255,255,0.3)] hover:bg-cyan-400 transition-colors text-lg flex items-center justify-center gap-2">
                      Confirmar Agendamento <span className="opacity-70 text-sm font-normal">({selectedTime})</span>
                    </button>
                  </Link>
                </div>
