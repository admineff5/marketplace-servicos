...
export default function TarefasPage() {
    ...
    const [listas, setListas] = useState([
        { id: 1, name: "As minhas tarefas", count: 2 },
        { id: 2, name: "Pendencia - Teste", count: 0 },
    ]);
    ...
    const [tarefas, setTarefas] = useState([
        ...
        { id: 2, title: "Testes o que fazer 2", completed: false, listId: 1, star: false },
        { id: 3, title: "Teste o que fazer 1", completed: true, listId: 1, star: false, completedAt: "segunda, 16/03" },
    ]);
    ...
    return (
        <div className="min-h-[calc(100vh-4rem)] p-4 sm:p-8 bg-gray-50/50 dark:bg-[#0a0a0a] flex gap-6">
            ...
            <div className="w-full md:w-64 shrink-0 flex flex-col bg-white dark:bg-[#111112] border border-gray-200 dark:border-gray-800 rounded-2xl p-4 shadow-sm h-full">
                <button
                    onClick={() => {
                ...
                <div className="space-y-1">
                    ...
                    >
                        <div className="flex items-center gap-3">
                            <ClipboardList className="w-4 h-4" /> Temas de Agendamentos
                        </div>
                    </button>
                    <button
                ...
                <div className="mt-8">
                    <p className="px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2">Listas</p>
                    <div className="space-y-1">
                        {listas.map((list) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-center justify-between w-full px-4 py-2.5 rounded-xl text-sm font-medium transition-colors ${activeListId === list.id ? "bg-cyan-50 text-cyan-700 dark:bg-primary/10 dark:text-primary" : "text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800"}`}
                                >
                                    <div className="flex items-center gap-3">
                    ...
                    {isCreatingList ? (
                        <div className="mt-2 px-4 flex items-center gap-2">
                            <input
                                ...
                                className="w-full bg-gray-50 dark:bg-gray-800 border-none rounded-lg px-3 py-1.5 text-xs text-gray-900 dark:text-white outline-none focus:ring-1 focus:ring-cyan-600 dark:focus:ring-primary"
                            />
                            <button onClick={addList} className="p-1 bg-cyan-700 dark:bg-primary text-white dark:text-gray-900 rounded-md"><Check className="w-3 h-3" /></button>
                        </div>
                    ) : (
                        <button
                            onClick={() => setIsCreatingList(true)}
                            className="flex items-center gap-2 px-4 py-2 mt-1 text-xs text-cyan-700 hover:text-cyan-800 dark:text-primary dark:hover:text-cyan-400 font-bold"
                        >
            ...
            <div className="flex-1 grid grid-cols-1 xl:grid-cols-2 gap-6 items-start">
                ...
                {(activeListId === null ? listas : listas.filter(l => l.id === activeListId)).map((lista) => {
                    const listTasks = tarefas.filter(t => t.listId === lista.id);
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                                ...
                                {addingTaskToListId === lista.id ? (
                                    ...
                                    <button
                                        ...
                                        className="flex items-center gap-3 w-full text-left text-sm text-gray-400 hover:text-cyan-700 dark:hover:text-primary font-medium py-1.5 cursor-pointer border-b border-dashed border-gray-100 dark:border-gray-800 pb-3"
                                    >
                                    ...
                                    {openTasks.map(task => (
                                        <div key={task.id} className="flex items-center justify-between group py-2 px-2 hover:bg-gray-50 dark:hover:bg-gray-800/50 rounded-xl transition-all">
                                            <div className="flex items-center gap-3 flex-1">
                                                <button onClick={() => toggleComplete(task.id)} className="w-5 h-5 rounded-full border border-gray-300 dark:border-gray-600 flex items-center justify-center hover:bg-cyan-50 dark:hover:bg-gray-700 transition-all">
                                                </button>
                                                <span className="text-sm text-gray-900 dark:text-gray-200">{task.title}</span>
                                            </div>
                                            <button onClick={() => toggleStar(task.id)} className={`opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md ${task.star ? "text-yellow-500 opacity-100" : "text-gray-400 hover:text-gray-600"}`}>
                                                ...
                                                {completedTasks.map(task => (
                                                    <div key={task.id} className="flex items-center justify-between group py-1.5 px-2 hover:bg-gray-50 dark:hover:bg-gray-800/20 rounded-lg transition-all">
                                                        <div className="flex items-center gap-3 flex-1 min-w-0">
                                                            <button onClick={() => toggleComplete(task.id)} className="w-5 h-5 rounded-full bg-cyan-100 dark:bg-primary/20 flex items-center justify-center text-cyan-700 dark:text-primary">
